...
// Reads straight from chrome.storage.local and aggregates in-page. No network
// request — this is what lets the extension work standalone with no server.
async function fetchStats(range) {
  const rollups = await AICM_STORE.readRollups();
  const stored = await chrome.storage.local.get(['aicm_device_id']);
  return AICM_STATS.computeStats(rollups, range, stored.aicm_device_id || 'this browser');
}
...
async function refresh() {
  try {
    ...
    console.error(err);
    document.getElementById('eventCount').textContent = 'Could not read local data — see the browser console for details.';
  }
}
...
async function renderStorageUsage() {
  try {
    const { bytes, quota } = await AICM_STORE.estimateUsage();
    const kb = (bytes / 1024).toFixed(0);
    const pct = ((bytes / quota) * 100).toFixed(1);
    document.getElementById('storageUsage').textContent = ` Currently using ${kb} KB (${pct}% of the space available to this extension).`;
  } catch (err) {
    /* usage display is optional — never let it break the page */
  }
}

document.getElementById('exportData').addEventListener('click', async () => {
  const data = await AICM_STORE.exportAll();
...
document.getElementById('clearData').addEventListener('click', async () => {
  // Deliberately blunt wording: this is unrecoverable, and the whole value of
  // the tool is accumulated history.
  const ok = window.confirm(
    'Delete all recorded AI consumption data?\n\nThis erases your entire history and cannot be undone. Consider downloading your data first.'
  );
  if (!ok) return;
  await AICM_STORE.clearAll();
  await refresh();
